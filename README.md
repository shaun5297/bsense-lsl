...
- 在长静息/休息结束时播放带 Marker 的离线中文女声提示；
...
5. 多选模块按 M0 → M1 → M2 → M3A → M3B → M4A → M4B → M5 执行；
...
| `m1_mi` | M1 运动想象（探索） | 左手、右手、空闲三分类及 Run 级主观评分 |
...
生成器固定使用 `zh-CN-XiaoxiaoNeural`，将联网生成的音频转换并裁剪为项目内置的 24 kHz 单声道 PCM WAV。实验程序在 Windows 使用 `winsound`、在 macOS 使用 `afplay` 播放缓存文件；文件缺失时回退到原系统提示音。
