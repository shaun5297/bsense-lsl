...
- 可独立或连续执行 M0、M1、M2、M3A、M3B、M4A、M4B、M6、M7、M5；
- 每个模块单独保存 XDF，结束后再决定是否进入下一模块；
- 自动记录 N-Back 反应时、正确性和主观评分；字母与中央十字分时显示；
- 提供赛道7“脑状态安检”预设：睡眠/班次背景、采前/采后 KSS、EEG 质量门控、睁眼基线、3 分钟 SART、独立 3 分钟 PVT 研究参照和四态结果；
...
macOS 需要带 Tk 的 Python 3.11–3.13；安装脚本会自动选择兼容解释器并创建本项目专用 `.venv`，缺失时会尝试通过 Homebrew 自动安装（未装 Homebrew 则需先安装）。默认数据目录是 `~/Documents/BCI/data/bsense`。完整的双机连接、权限设置和验证流程见 [macOS 使用说明](docs/MACOS.md)。
...
它会使用 Python 3.13 创建仓库内的 `.venv`，安装固定版本依赖并运行协议自测。未安装 Python 3.13 时脚本会尝试通过 winget 自动安装（无 winget 时给出手动安装指引）；损坏或来自其他操作系统的虚拟环境会自动移开（`.venv.bak.*`）并重建，虚拟环境不能在 macOS、Linux 和 Windows 之间复制。
...
6. 多选模块按 M0 → M1 → M2 → M3A → M3B → M4A → M4B → M6 → M7 → M5 执行；
...
| 采集批次 | 选择模块 | 自动计时 | 用途 |
|---|---|---:|---|
| A：探索性运动与意图 | M0、M1、M4A、M4B | 约 48.7 分钟 | 运动想象、提示后意图和目标注意 |
| B：核心认知与疲劳 | M0、M2、M3A、M3B、M5 | 约 51.7 分钟，另加 M5 问卷 | 认知负荷、安全动作、疲劳和结束评估 |
| 赛道7：脑状态安检 | M6 | 研究标签模式约 7.8 分钟，关闭后约 4.7 分钟，另加表单 | 上岗前当班状态筛查；正式先导开启独立 PVT，比赛演示可关闭 |
...
应用批次预设会自动关闭短流程并清除未属于该批次的模块。同一天、同一次佩戴内连续执行多个批次时，可勾选“本日同会话已完成 M0，跳过基线模块”，后续批次不再重复 M0；跨日或重新佩戴后不得跳过，必须重新执行完整 M0。每条 JSON Marker 会记录 `acquisition_batch`、`m0_skipped`、`short_protocol` 和 `older_adult_timing`，因此即使数据跨 Session 保存，也能审计当时实际使用的批次和计时模式。
...
| `m6_readiness` | M6 脑状态安检 | 详细背景、双 KSS、EEG 质量门控、睁眼基线、3 分钟 SART、独立 PVT 参照与四态结果 |
| `m7_p300` | M7 机器狗指令 P300（探索） | 方向宫格、四阶段控制回合（提示→闪烁→下发→反馈）、100 ms 高亮、175 ms SOA 和 NASA-TLX |
| `m5_debrief` | M5 结束问卷 | 困倦、任务体验、设备舒适度与不适记录 |

M4A/M4B 从 `assets` 加载水杯、手机和药瓶 PNG；图片缺失时自动回退到大字号文字，不影响 Marker。

设备能力的保守边界是：Fp1/Fp2 不覆盖典型感觉运动区或顶叶 P300 最优位置，额部 fNIRS 也不适合判定单次快速高亮。因此 M1、M4A、M4B、M7 均按探索性模块管理；M2/M3B 是主要验证方向。任何比赛准确率都必须来自按被试划分的留出评估，不能由范式直接预估。
...
- 本项目用于研究和比赛，不提供医疗诊断。
- M6 只向管理角色输出“正常、建议复测、建议休息、无法评估”及操作建议；原始脑信号、反应明细和身份资料不得用于处罚、自动上岗决策或永久能力画像。
...
- [M6 脑状态安检验证方案](docs/READINESS_VALIDATION.md)
