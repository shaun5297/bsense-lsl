...
| 批次 | 模块 | 自动计时 | 设计目的 |
|---|---|---:|---|
| A：探索性运动与意图 | M0 → M1 → M4A → M4B | 约 52.4 分钟 | 将运动想象、提示后意图和探索性目标注意放在同一批 |
| B：核心认知与疲劳 | M0 → M2 → M3A → M3B → M5 | 约 51.6 分钟，另加 M5 问卷 | 采集认知负荷与疲劳；M3B 位于有效任务末端，避免疲劳污染其他主要任务 |
...
1. M0 → M1 → M4A，自动计时约 43.6 分钟；
2. M0 → M2 → M4B，自动计时约 44.8 分钟；
3. M0 → M3A → M3B → M5，自动计时约 21.2 分钟，另加 M5 问卷。

分批不是把一次中断的模块续接到下一次：每个模块仍应完整执行并生成独立 XDF。跨批次不得沿用上一批次的 M0，也不要把 M3B 安排在仍需比较清醒状态的任务之前。
