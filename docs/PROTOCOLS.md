...
| M1 | 28.5 分钟 | 引导练习 → 4 Run × 30 Trial；每 Run 三类各 10 次并随机打乱，Run 后评分 |
...
M0–M5 自动计时合计约 98.3 分钟；加 10 分钟中场休息、M5 及各模块人工评分后，现场应预留约 2 小时。M1 的方案标称 40 分钟包含设备调整和组间休息等现场时间；程序内自动时序约 28.5 分钟。每个 Run 三类严格平衡，且不会出现同一条件连续 3 次。启用老年被试节奏后，执行想象延长至 5 秒、试次休息延长至 4 秒、组间休息延长至 5 分钟，自动计时约 39.5 分钟。
...
每个正式试次总长 2 秒：字母显示 0.5 秒，随后显示中央注视十字 1.5 秒；整个 2 秒窗口内都可以按空格响应。这样即使相邻字母相同，也有明确的视觉边界。0-back 仅目标刺激使用 `X`，非目标字母池明确排除 `X`，不会再出现相同画面对应冲突标签。M2 的每个 Block 和 M3B 的每个两分钟段均固定 25% 目标刺激，避免随机波动造成类别严重失衡。每个刺激都会生成 `nback_stimulus`，按键时生成 `nback_response`，刺激窗口结束时生成 `nback_trial_result`。结果包含：
...
首页可启用按键后的绿色“正确”/红色“错误”反馈。该选项默认关闭，只用于练习和界面联调；正式采集开启反馈会改变后续视觉诱发反应与任务策略，因此应保持关闭。每次按键 Marker 都记录 `feedback_shown`，便于审计。
