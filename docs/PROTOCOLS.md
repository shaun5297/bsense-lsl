...
- M7 使用更快的六宫格单刺激 P300 时序，但同样受 Fp1/Fp2 位置限制；逐闪烁 EEG 标签仅供事件相关探索，额部 fNIRS 不用于单次闪烁分类；
...
| M7 | 75.3 分钟 | 3 Block × 6 目标 × 20 Trial × 10 Sequence；100 ms 高亮、175 ms SOA，Block 间休息 2 分钟 |
...
设计方案中“约 45 分钟”的现场排程与上述正式参数不一致：按 175 ms × 6 × 10 × 360，仅闪烁时间已经约 63 分钟，程序自动计时约 75.3 分钟。因此 M7 应作为独立采集模块安排；如需压缩时长，必须先修订并冻结 Trial/Block 参数，而不能采集中临时跳过。
