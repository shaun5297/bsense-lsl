...
响应小于 100 ms 单独标为 `false_start`，不再混入正常反应时；Go 无响应为 `omission`，No-Go 错按为 `commission`（含 No-Go 上的抢按：其 `outcome` 归为 `commission`，同时保留 `false_start` 标志并计入抢按率），No-Go 正确抑制为 `correct_rejection`。练习试次和操作员标记 `trial_invalid` 的正式试次不进入计分；PVT 中无效标记作用于最近一条结果（刺激呈现中为当前试次，刺激间隙为最近结果行，最近结果是等待期抢按时只作废该抢按行，不回退到上一刺激试次）。首次检测出现任何风险阈值只输出“建议复测”，不会直接升级为“建议休息”；只有复测且高风险信号仍存在时才可输出“建议休息”。EEG 质量不合格、质量采样缺失或正式试次不足 90% 时输出“无法评估”，不能把缺失数据误判为正常。M6 的 SART 节奏固定为每试次 1 秒，不随“老年被试节奏”设置变化。
...
2. **闪烁选择**：5 个 Sequence，每个 Sequence 六格各高亮一次，高亮 100 ms、SOA 175 ms。选择阶段不再显示目标边框与指令名（被试在提示阶段记住目标），避免 target 闪烁画面与非 target 存在可区分的低层视觉特征造成标签泄漏；目标信息仍完整保留在 Marker 元数据中；
